...
tab1, tab2=st.tabs(['Model Comparison','Station Predictions'])
...
with tab2:
    ...
    st.write('Ridership prediction with Polynomial Regressor(degree2)',poly_regression.predict(poly.fit_transform(input_data.values))[0])
    st.write('Ridership prediction with Random Forest Regressor',forest.predict(input_data.values)[0])
    st.write('Ridership prediction with Gradient Boosting Regressor',boosting_r.predict(input_data.values)[0])